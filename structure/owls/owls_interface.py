...
def setup(options):
	...
	if mode=="single":
		...
		try:
			...
			raise ValueError("Please set the parameter nterm in the owls section of the ini file to use Chebyshev OWLS")
		baryonPowerModulator = owls.ChebyshevBaryonPowerModulator(nterm=nterm, extremes=1.0)
		print "Using checbyshev mode with %d terms" % nterm
	elif mode=="fixed":
		try:
			powtable = options[option_section,'powtable']
		except KeyError:
			raise ValueError("Please set the parameter powtable in the owls section of the ini file, pointing to an OWLS powtable file to use")
		...
		raise ValueError("'mode' must be either 'single' (one parameter, OWLS_R), 'chebyshev' (nterm parameters OWLS_R1,...), 'fixed' (no params; fixed OWLS sim), or 'scaled' (one parameter, OWLS_R) ")
...
def load_parameters(block, modulator):
	section="OWLS"
	if isinstance(modulator, owls.ChebyshevBaryonPowerModulator):
		rk = [block[section, "K%d"%i] for i in xrange(1,modulator.nterm+1)]
		rz = [block[section, "Z%d"%i] for i in xrange(1,modulator.nterm+1)]
		r = np.concatenate([rk, rz])
	elif isinstance(modulator, owls.FixedBaryonPowerModulator):
		r=None
	elif isinstance(modulator, owls.BaryonPowerModulator):
