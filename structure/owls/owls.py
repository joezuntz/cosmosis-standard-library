...
try:
	...
	sys.stderr.write("The OWLS baryon power code requires the scipy python module\n")
	sys.stderr.write("but it could not be found.  This code will fail.\n")
	raise ImportError("Require scipy module to run baryon code.  Could not find it.")
...
class OwlsFileUser(object):
	...
	def _load_file(filename):
		...
		for i, z_i in enumerate(out_z):
			...
			k_i = np.log10(k_i[ok])
			P_i = np.log10(P_i[ok])
			S = scipy.interpolate.InterpolatedUnivariateSpline(k_i, P_i)#, s=3)
			P_fit = S(out_k)
			out_P[:,i] = P_fit
...
class ChebyshevBaryonPowerModulator(BaryonPowerModulator):
	...
	def modulate(self, k, z, P, r):
		...
		r_k = r[:self.nterm]
		r_z = r[self.nterm:]
		assert len(r_k)==len(r_z)==self.nterm, "Parameters not found correctly for OWLS code"
		logk = np.log10(k)
		d = self.dm(logk, z)
...
class FixedBaryonPowerModulator(OwlsFileUser):
	...
	def modulate(self, k, z, P, r=None):
		...
		# It is ignored
		logk = np.log10(k)
		s = (10**self.sim(logk, z) - 10**self.dm(logk,z))/(10**self.dm(logk,z))
		s[k<self.sim_k[0]]=0.
