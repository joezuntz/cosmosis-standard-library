...
void emuInit(const char * filename, emu_params * par) {
    ...
    for(i=0; i<peta; i++) {
        ...
        for(j=0; j<m; j++) {
            ...
            // Diagonal term
            gsl_matrix_set(SigmaSim, j, j, (1.0/par->lamz[i]) + (1.0/par->lamws[i]));
            ...
            for(k=0; k<j; k++) {
                ...
                cov = 0.0;
                for(l=0; l<p; l++) {
                    cov -= par->beta[i][l]*pow(par->x[j][l] - par->x[k][l], 2.0);
                } // for(l=0; l<p; l++)
                cov = exp(cov) / par->lamz[i];
        ...
        for(j=0; j<m; j++) {
            par->KrigBasis[i][j] = gsl_vector_get(b, j);
        }
...
int emu(emu_params * par, double *xstar, double *ystar) {
    ...
    for(i=0; i<p; i++) {
        if((xstar[i] < par->xmin[i]) || (xstar[i] > par->xmax[i])) {
            switch(i) {
                case 0:
                    printf("omega_m must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 1:
                    printf("omega_b must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 2:
                    printf("sigma_8 must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 3:
                    printf("h must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 4:
                    printf("n_s must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 5:
                    printf("w_0 must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 6:
                    printf("(-w_0-w_a)^(1/4) must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
                case 7:
                    printf("omega_nu must be between %lf and %lf.\n", par->xmin[i], par->xmax[i]);
                    break;
            }
            return 1;
        }
    ...
    if((xstar[p] < par->z[rs-1]) || (xstar[p] > par->z[0])) {
    ...
    for(i=0; i<p; i++) {
        xstarstd[i] = (xstar[i] - par->xmin[i]) / par->xrange[i];
    }
    ...
    for(i=0; i<peta; i++) {
        for(j=0; j<m; j++) {
            logc = 0.0;
            for(k=0; k<p; k++) {
                logc -= par->beta[i][k]*pow(par->x[j][k] - xstarstd[k], 2.0);
            }
            Sigmastar[i][j] = exp(logc) / par->lamz[i];
        }
    }
    ...
    for(i=0; i<peta; i++) {
        wstar[i]=0.0;
        for(j=0; j<m; j++) {
            wstar[i] += Sigmastar[i][j] * par->KrigBasis[i][j];
        }
    }
    ...
    for(i=0; i<neta; i++) {
        ystaremu[i] = 0.0;
        for(j=0; j<peta; j++) {
            ystaremu[i] += par->K[i][j]*wstar[j];
        }
        ystaremu[i] = ystaremu[i]*par->sd + par->mean[i];
    }
    ...
    if(zmatch == -1) {
        for(i=0; i<nmode; i++) {
            ...
            gsl_spline_init(zinterp, par->z_asc, ybyz, rs);
    ...
    // Convert to P(k)
    for(i=0; i<nmode; i++) {
        ystar[i] = ystar[i] - 1.5*log10(par->mode[i]) + log10(2) + 2*log10(M_PI);
...
//     emu_params * par = malloc(sizeof(emu_params));
//     emuInit("P_tot.dat", par);
