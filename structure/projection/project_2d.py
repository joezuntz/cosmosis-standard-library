...
class Spectrum(object):
    ...
    def magnification_prefactor(self, block, bin):
        ...
        alpha = np.atleast_1d(
            np.array(block[names.galaxy_luminosity_function, "alpha_binned"]))
...
class SpectrumCalculator(object):
    ...
    def __init__(self, options):
        ...
        self.fatal_errors = options.get_bool(
            option_section, "fatal_errors", False)
        self.get_kernel_peaks = options.get_bool(
            option_section, "get_kernel_peaks", False)
        self.save_kernel_zmax = options.get_double(
            option_section, "save_kernel_zmax", -1.0)
        ...
        self.absolute_tolerance = options.get_double(
            option_section, "limber_abs_tol", 1e-5)
        self.relative_tolerance = options.get_double(
            option_section, "limber_rel_tol", 1e-3)
    ...
    def parse_requested_spectra(self, options):
        ...
        for spectrum in self.spectrumType:
            ...
            if not value:
                raise ValueError(
                    "Empty value for option {} in project_2d.".format(name))
            ...
            for value in values:
                try:
                    ...
                    self.req_spectra.append(spectrum.value(
                        self, kernel_a, kernel_b, save_name))
                except:
                    raise ValueError(
                        "To specify a P(k)->C_ell projection with one or more sets of two different n(z) samples use the form shear-shear=sample1-sample2 sample3-sample4 ....  Otherwise just use shear-shear=T to use the standard form.")
    ...
    def compute_spectra(self, block, spectrum):
        ...
        for i in range(na):
            ...
            for j in range(jmax):
                ...
                block[spectrum_name, 'bin_{}_{}'.format(
                    i + 1, j + 1)] = c_ell(self.ell)
                if self.get_kernel_peaks:
                    chi_peak, z_peak = spectrum.kernel_peak(
                        block, i, j, self.a_of_chi)
                    block[spectrum_name, "chi_peak_{}_{}".format(
                        i + 1, j + 1)] = chi_peak
