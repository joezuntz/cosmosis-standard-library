...
fastpt=FASTPT.FASTPT(k,to_do=['IA_ta'],low_extrap=-6,high_extrap=4,n_pad=n_pad) 
	
	
t1=time()	
IA_deltaE1, IA_deltaE2, IA_0E0E, IA_0B0B=fastpt.IA_ta(P,C_window=C_window) 
t2=time()
...
IA_deltaE1=IA_deltaE1[98:599]
IA_deltaE2=IA_deltaE2[98:599]
IA_0E0E=IA_0E0E[98:599]
IA_0B0B=IA_0B0B[98:599]
k=k[98:599]
...
ax1.plot(k,abs(IA_deltaE1), lw=4, color='black', label = r'$P_{\delta E1}(k)$')
ax1.plot(k,abs(IA_deltaE2), lw=4, color='red', label = r'$P_{\delta E2}(k)$')
ax1.plot(k,abs(IA_0E0E), '--', lw=4, color='blue', label = r'$P_{0E0E}^{EE}(k)$')
ax1.plot(k,abs(IA_0B0B), '--', lw=4, color='green', label = r'$P_{0B0B}^{BB}(k)$')
