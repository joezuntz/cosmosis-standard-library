...
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.set_ylabel(r'RSD $A_i^{\rm I}(k)$', size=30)
...
ax1.plot(k,A1, lw=4, color='black', label=r'$A_1^{\rm I}(k)$, $f=%2.1f$'%(f))
...
ax2.plot(k,A1/(d[:,7]+f*d[:,8])-1,lw=2, color='black', alpha=.5, label='fractional difference')
...
ax3.plot(k,A3, lw=4, color='black', label=r'$A_3^{\rm I}(k)$, $f=%2.1f$'%(f))
...
ax4.plot(k,A3/(f*d[:,9]+f**2 *d[:,10])-1,lw=2, color='black', alpha=.5, label='fractional difference')
...
ax5.plot(k,A5, lw=4, color='black', label=r'$A_5^{\rm I}(k)$, $f=%2.1f$'%(f))
...
ax6.plot(k,A5/(f**2 *d[:,11])-1,lw=2,color='black', alpha=.5, label='fractional difference')
