...
class FASTPT:
	...
	def IA_mix(self,P,P_window=None,C_window=None):
		...
		return 2*P_A, 4*P_Btype2, 2*P_DEE, 2*P_DBB
         ## eq 18; eq 19; eq 21 EE; eq 22 BB
		 #### This now has the entire pre-factor for all of the terms, including P_Btype2. Xiao, have you updated the kernel in the internal function to remove the factor of 2?
	...
	def IRres(self,P,L=0.2,h=0.67,rsdrag=135,P_window=None,C_window=None):
		...
		Sigma = integrate.quad(lambda x: (4*pi)*psmooth(x)*(1-3*(2*rbao*x*cos(x*rbao)+(-2+rbao**2*x**2)*sin(rbao*x))/(x*rbao)**3)/(3*(2*pi)**3), kmin, L)[0]
