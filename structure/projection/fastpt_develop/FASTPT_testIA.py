...
class FASTPT:
	...
	def IA_ABD(self,P,P_window=None,C_window=None):
...
if __name__ == "__main__":
	...
	d=np.loadtxt('PT_1loop_IA_terms4_planck15_trim.dat')
	...
	#calculate various IA P(k)
	P_IA=fastpt.IA(P,C_window=C_window)
	P_IA_ABD=fastpt.IA_ABD(P,C_window=C_window)
	...
	ax.plot(k,P,label='linear')
	ax.plot(k,P_IA[0], label=r'$P_{EE}$' )
	ax.plot(k,P_IA[1], label=r'$P_{BB}$' )
#	ax.plot(k,P_IA_ABD[0], label=r'$A$' )
#	ax.plot(k,abs(P_IA_ABD[1]), label=r'$B$' )
#	ax.plot(k,P_IA_ABD[2], label=r'$D_{EE}$' )
#	ax.plot(k,P_IA_ABD[3], label=r'$D_{BB}$' )		
	
	ax.plot(d[:,0],d[:,1],'--', label=r'$test$' )
	ax.plot(d[:,0],0.5*d[:,12],'--', label=r'$test$' )
	ax.plot(d[:,0],0.5*d[:,13],'--', label=r'$test$' )
#	ax.plot(d[:,0],d[:,16],'--', label=r'$test$' )
#	ax.plot(d[:,0],abs(d[:,18]*d[:,1]),'--', label=r'$test$' )
#	ax.plot(d[:,0],d[:,19],'--', label=r'$test$' )
#	ax.plot(d[:,0],d[:,20],'--', label=r'$test$' )		

# Factor of 2 between my output code and FAST-PT output. Probably due to combinatorics, but CONFIRM.
	...
	ax.set_xscale('log')
#	ax.set_yscale('log')
	ax.set_ylabel(r'$P(k) ratio$', size=30)
	ax.set_xlabel(r'$k$', size=30)

	ax.plot(k,P/d[:,1],label='linear')
	ax.plot(k,P_IA[0]/(0.5*d[:,12]), label=r'$P_{EE}$' )
	ax.plot(k,P_IA[1]/(0.5*d[:,13]), '--',label=r'$P_{BB}$' )
	ax.plot(k,P_IA_ABD[0]/d[:,16], label=r'$A$' )
	ax.plot(k,P_IA_ABD[1]/(d[:,18]*d[:,1]), label=r'$B$' )
	ax.plot(k,P_IA_ABD[2]/d[:,19], label=r'$D_{EE}$' )
	ax.plot(k,P_IA_ABD[3]/d[:,20], '--',label=r'$D_{BB}$' )		
