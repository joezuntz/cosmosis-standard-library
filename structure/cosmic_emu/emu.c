...
int emu(double *xstar, double *ystar) {
    ...
    for(i=0; i<p; i++) {
        if((xstar[i] < xmin[i]) || (xstar[i] > xmax[i])) {
            switch(i) {
                case 0:
                    fprintf(stderr,"omega_m must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 1:
                    fprintf(stderr,"omega_b must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 2:
                    fprintf(stderr,"sigma_8 must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 3:
                    fprintf(stderr,"h must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 4:
                    fprintf(stderr,"n_s must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 5:
                    fprintf(stderr,"w_0 must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 6:
                    fprintf(stderr,"(-w_0-w_a)^(1/4) must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
                case 7:
                    fprintf(stderr,"omega_nu must be between %lf and %lf. (%lf)\n", xmin[i], xmax[i], xstar[i]);
                    break;
            }
            return 1;
        }
    ...
    if((xstar[p] < z[0]) || (xstar[p] > z[rs-1])) {
        fprintf(stderr, "CosmicEmu: z must be between %f and %f.\n", z[0], z[rs-1]);
        fprintf(stderr, "You set z = %f\n", xstar[p]);
        return 2;
    }
