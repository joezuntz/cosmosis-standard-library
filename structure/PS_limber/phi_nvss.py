...
import gals_nvss_kernel as gals
...
def execute(block, config):
    ...
    gkern = gals.kern(omega_m, h0 , hspline ,zdist)

    zmax = 15.
    zmin = 0.
    lbins = np.arange(2,1000,10)

    cl = np.zeros(np.size(lbins))

    # Compute Cl implicit loops on ell
    # =======================
    cldd = [cl_limber_z(chispline, hspline, rbs, l, lkern, k2=None, zmin=zmin, zmax=zmax)for l in lbins]
    print 'lensig done'
    cldg = [cl_limber_z(chispline, hspline, rbs, l, lkern, gkern, zmin=zmin, zmax=zmax)for l in lbins]
    print 'lensig x CIB done'
    clgg = [cl_limber_z(chispline, hspline, rbs, l, gkern, zmin=0., zmax=6.)for l in lbins]
    print 'CIB x CIB done'
    ...
    block[section, "clgg_"+obj] = clgg
    block[section, "cldg_"+obj] = cldg
    block[section, "cldd_"+obj] = cldd
  ...
  # # NVSS shot noise in gal/sr, from Fig. 1 of 0705.3980
  #   nlgg    = 1./159000. * np.ones(lmax_dg+1)


  #   print 'Expect detection S/N = ', np.sqrt( fsky * np.sum((2*l+1) * cldg[l]**2 / ((cldd[l])*(clgg[l]+nlgg[l])) for l in xrange(lmin_dg,lmax_dg+1)) )
