...
def load_histogram_form(ext, bin, upsampling):
    ...
    norm = np.sum(nz)  # norm = np.trapz(nz, z)
    mbias = norm-1.
    nz /= norm
...
def setup(options):
    ...
    config['nz'] = nz
    config['multiplicative_bias'] = multiplicative_bias
...
def execute(block, config):
    ...
    nz = config['nz']
    multiplicative_bias = config['multiplicative_bias']
    ...
    mbias = multiplicative_bias[index]
