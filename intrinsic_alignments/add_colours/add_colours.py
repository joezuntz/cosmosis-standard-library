...
def execute(block, config):
	...
	P_II = f_red*f_red * A_red*A_red * P_II_red * (1+z_grid)**(2.*alpha_red)
	P_II += f_blue*f_blue * A_blue*A_blue * P_II_blue * (1+z_grid)**(2.*alpha_blue)
	P_II += f_red*f_blue * A_red*A_blue * P_II_red_blue * (1+z_grid)**(alpha_blue)*(1+z_grid)**(alpha_red)

	P_GI = f_red * A_red * P_GI_red * (1+z_grid)**(alpha_red)
	P_GI = f_blue * A_blue * P_GI_blue * (1+z_grid)**(alpha_blue)

	block.put_grid(section_ia, "z", z, "k_h", k, "P_II_%s"%catalogue, P_II)
	print "Saved II spectrum for %s"%catalogue
	block.put_grid(section_ia, "z", z, "k_h", k, "P_GI_%s"%catalogue, P_GI)
