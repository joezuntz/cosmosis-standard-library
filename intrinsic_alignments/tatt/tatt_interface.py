...
class Pk_interp(object):
    def __init__(self,ks,Pks):
        if np.all(Pks>0):
            self.interp_func=interp.interp1d(np.log(ks),np.log(Pks),bounds_error=False,fill_value=-np.inf)
            self.interp_type='loglog'
        elif np.all(Pks<0):
            self.interp_func=interp.interp1d(np.log(ks),np.log(-Pks),bounds_error=False,fill_value=-np.inf)
            self.interp_type='minus_loglog'
        else:
            self.interp_func=interp.interp1d(np.log(ks),Pks,bounds_error=False,fill_value=0.)
...
def read_precomp_file(filename, sub_const=True):
    # 0:k 1:Plin 2:P22 3:P13 4:d|dfd_1 5:d|dfd_2 6:dd|d 7:dd|dfd 8:dfd|dfd_EEnolowk 9:dfd|dfd_BBnolowk 10:dfd|dfd_EE 11:dfd|dfd_BB 12:tt_II_EE 13:tt_II_BB
    # 14:tt_II_EE_nolowk 15:tt_II_BB_nolowk 16:A 17:B 18:B_nolowk 19:D_EE 20:D_BB 21:D_EE_nolowk 22:D_EE_nolowk
...
def get_IA_terms(block, k_out, k_nl, z_out, P_lin, P_nl, Dz, A1, A2, Adel, bias_ta, bias_tt,
                alpha1, alpha2, alphadel, z_piv, Omega_m, precomp_file=None, s8_rescale_factor=None,
                sub_const=True, include_s2_terms=False, sigma2_S=None, sub_lowk=False):
...
#    assert P_lin.shape[1]==P_nl.shape[1]==len(k_out) #this is no longer necessary
    if precomp_file:
        ...
        if True:
            ...
            #get from the block
            P_IA_dict_block={}
            for key in ['P_tt_EE','P_tt_BB','P_ta_dE1','P_ta_dE2','P_ta_EE','P_ta_BB','P_mix_A','P_mix_B','P_mix_D_EE','P_mix_D_BB']:
    ...
    C1 = -1.*A1*C1_RHOCRIT*Omega_m/Dz*((1.+z_out)/(1.+z_piv))**alpha1
    Cdel = -1.*Adel*C1_RHOCRIT*Omega_m/Dz*((1.+z_out)/(1.+z_piv))**alphadel
    C2 = 5*A2*C1_RHOCRIT*Omega_m/Dz**2*((1.+z_out)/(1.+z_piv))**alpha2
    ##output for comparison##
    #out_arr = np.zeros((len(C1),4))
    #out_arr[:,0] = z_out
    #out_arr[:,1] = Dz
    #out_arr[:,2] = C1
    #out_arr[:,3] = C2
    #np.savetxt('temp_out_C1C2.txt',out_arr)
...
#    bias_ta = amp_3d(bias_ta, len(Dz), len(k_use))
#    bias_tt = amp_3d(bias_tt, len(Dz), len(k_use))
	

    #Get nonlinear Pk at the fast-pt k grid. (lin Pk is already read-in at the fast-pt k grid from the fastpt block)
    #P_IA_dict['P_nl']=resample_power(P_IA_dict['Plin'], P_nl, k_IA, k_nl)
    ## Currently no need to do this re-sample, since everything from the block is put onto the NL k grid above.
    P_IA_dict['P_nl']=P_nl

    #LA/NLA terms not included here...
    #print "bias_ta, P_IA_dict['P_ta_EE'], P_IA_dict['P_ta_dE1'], P_IA_dict['P_ta_dE2']"
    #print bias_ta, P_IA_dict['P_ta_EE'], P_IA_dict['P_ta_dE1'], P_IA_dict['P_ta_dE2']
    
    
    P_IA_out={}
    p_ta_ii_EE = Cdel**2 * P_IA_dict['P_ta_EE'] + C1*Cdel * (2*P_IA_dict['P_ta_dE1'] + 2*P_IA_dict['P_ta_dE2'])
    p_ta_ii_BB = Cdel**2 * P_IA_dict['P_ta_BB']
    p_ta_gi = Cdel * (P_IA_dict['P_ta_dE1'] + P_IA_dict['P_ta_dE2'])
    if include_s2_terms:
        p_ta_gi += Cdel * (58./105)*sigma2_S*P_IA_dict['Plin']
        p_ta_ii_EE += 2 * C1*Cdel * (58./105)*sigma2_S*P_IA_dict['Plin']
    ...
    #mixed
    p_mix_ii_EE = 2.* C2 *(C1 * P_IA_dict['P_mix_A'] + C1 * P_IA_dict['P_mix_B'] + Cdel * P_IA_dict['P_mix_D_EE'])
    p_mix_ii_BB = 2.* Cdel * C2 * P_IA_dict['P_mix_D_BB']
...
def setup(options):
    ...
    if Asigma8:
        print('A1*sigma_8, A2*sigma_8^2, Adel*sigma_8^2 are the parameters being sampled')
    if name:
        suffix = "_" + name
    else:
        suffix = ""
    return sub_const, sub_lowk, include_s2_terms, precomp_file, ia_model, suffix, gal_intrinsic_power, use_same_bias, no_IA_E, no_IA_B, Asigma8

def execute(block, config):

    sub_const, sub_lowk, include_s2_terms,precomp_file, ia_model, suffix, gal_intrinsic_power, use_same_bias, no_IA_E, no_IA_B, Asigma8 = config
...
#    p_nl = resample_power(p_lin, p_nl, k_lin, k_nl)
#    k_nl = k_lin
    assert (z_nl==z_lin).all(), "Expected identical z values for matter power NL & Linear in IA code"
    ...
    if no_IA_E:
        E_factor = 0.
        # print('not including IA E modes')
    if no_IA_B:
        B_factor = 0.
        # print('not including IA B modes')

    #read values these from the block
    if ('intrinsic_alignment_parameters','C1') in block:
        raise ValueError('Deprecated TATT parameter specified: ' + 'C1')
    if ('intrinsic_alignment_parameters','C2') in block:
        raise ValueError('Deprecated TATT parameter specified: ' + 'C2')
    A1 = block.get_double('intrinsic_alignment_parameters','A1', 1.)
    A2 = block.get_double('intrinsic_alignment_parameters','A2', 1.)
    alpha1 = block.get_double('intrinsic_alignment_parameters','alpha1', 0.)
    alpha2 = block.get_double('intrinsic_alignment_parameters','alpha2', 0.)
    alphadel = block.get_double('intrinsic_alignment_parameters','alphadel', alpha1)
    z_piv = block.get_double('intrinsic_alignment_parameters','z_piv', 0.)
    if ('intrinsic_alignment_parameters','Adel') in block:
        if ('intrinsic_alignment_parameters','bias_ta') in block:
            raise ValueError('bias_ta is not used when Adel is specified.')
        else:
            Adel = block.get_double('intrinsic_alignment_parameters','Adel', 1.)
            bias_ta = bias_tt = 1.0
    else:
        bias_ta = block.get_double('intrinsic_alignment_parameters','bias_ta', 1.)
        bias_tt = block.get_double('intrinsic_alignment_parameters','bias_tt', 1.)
        if use_same_bias: bias_tt = bias_ta
        Adel = bias_ta*A1
    ...
    if Asigma8:
        ...
        Adel = Adel/sigma_8**2
    # if use_same_bias:
    #     print('using same source bias for TA and TT')
    #     print('bias_ta = bias_tt =',bias_ta)
    # else:
    #     print('NOT using same bias')
    #     print('bias_ta, bias_tt =',bias_ta,bias_tt)
    #  # do we want to read in actual bias? what about lens bias for GI terms?
    
    # print('IA model is ',ia_model)
    IA_terms,k_use = get_IA_terms(block, k_lin, k_nl, z_lin, p_lin, p_nl, Dz, A1, A2, Adel, bias_ta, bias_tt,
                                    alpha1, alpha2, alphadel, z_piv, omega_m, precomp_file=precomp_file,
                                    sub_const=sub_const, include_s2_terms=include_s2_terms, sub_lowk=sub_lowk)

##### complete the proper IA model defintions.
    if (ia_model=='lin'):
        ii_ee_total = E_factor*IA_terms['lin_II_EE']
        ii_bb_total = 0.*IA_terms['ta_II_BB'] #can we pass a scalar to the block?
        ...
        # this is purely tidal torque. In a realistic scenario, it should probably include a tidal-alignment-like term (due to renormalization)
        ii_ee_total = E_factor*IA_terms['tt_II_EE']
        ii_bb_total = B_factor*IA_terms['tt_II_BB']
        gi_e_total = E_factor*IA_terms['tt_GI']
    elif (ia_model=='tatt'):
        ii_ee_total = E_factor*(IA_terms['nla_II_EE'] + IA_terms['ta_II_EE'] + IA_terms['tt_II_EE'] + IA_terms['mix_II_EE'])
        ii_bb_total = B_factor*(IA_terms['ta_II_BB'] + IA_terms['tt_II_BB'] + IA_terms['mix_II_BB'])
        gi_e_total = E_factor*(IA_terms['nla_GI'] + IA_terms['ta_GI'] + IA_terms['tt_GI'])  #no mix contribution to GI
    elif (ia_model=='mixed_only'):
        #not a physically sensible option, but useful for showing the extra contribution we get by considering the cross-terms....
...
# this block is for diagnostics. The E and B prefactors are NOT applied, since these are not used in the actual calculations.
    ii_ee_total_lin = IA_terms['lin_II_EE']
    ii_bb_total_lin = 0.*IA_terms['ta_II_BB'] #can we pass a scalar to the block?
    gi_e_total_lin = IA_terms['lin_GI']

    ii_ee_total_nla = IA_terms['nla_II_EE']
    ii_bb_total_nla = 0.*IA_terms['ta_II_BB'] #can we pass a scalar to the block?
    gi_e_total_nla = IA_terms['nla_GI']
    
    ii_ee_total_ta = IA_terms['nla_II_EE'] + IA_terms['ta_II_EE']
    ii_bb_total_ta = IA_terms['ta_II_BB']
    gi_e_total_ta = IA_terms['nla_GI'] + IA_terms['ta_GI']
    
    ii_ee_total_tt = IA_terms['tt_II_EE']
    ii_bb_total_tt = IA_terms['tt_II_BB']
    gi_e_total_tt = IA_terms['tt_GI']
    
    ii_ee_total_tatt = IA_terms['nla_II_EE'] + IA_terms['ta_II_EE'] + IA_terms['tt_II_EE'] + IA_terms['mix_II_EE']
    ii_bb_total_tatt = IA_terms['ta_II_BB'] + IA_terms['tt_II_BB'] + IA_terms['mix_II_BB']
    gi_e_total_tatt = IA_terms['nla_GI'] + IA_terms['ta_GI'] + IA_terms['tt_GI']  #no mix contribution to GI
    
    ii_ee_total_mixed_onlu = IA_terms['mix_II_EE']
    ii_bb_total_mixed_only = IA_terms['mix_II_BB']
    gi_e_total_mixed_only = np.zeros_like(ii_ee_total)
#    ############
    
#    print k_use
#    print (ii_ee_total_tatt[0]/ii_ee_total_nla[0])
#    print len(k_use)
#    print len(ii_ee_total_tatt[0])
#    print (ii_bb_total_tatt/ii_bb_total_nla)
#    print (gi_e_total_tatt/gi_e_total_nla)
    #plt.show()
    
    # plt.semilogx(k_use,ii_ee_total_tatt[0]/ii_ee_total_nla[0])
    # plt.savefig('pk.png')
    ...
    if gal_intrinsic_power:
        ...
        ##currently has no bias applied -> will need to be applied later. Thus, only linear bias can be applied.
