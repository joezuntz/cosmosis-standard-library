...
class XipmLikelihood(object):
    def __init__(self, covmat_file, xipm_data, n_z_bins,cov_num_rlzn=None,theta_mins=None,theta_maxs=None,plus_only=False,pmc=False):
        ...
        self.n_z_bin_pairs=self.n_z_bins*(self.n_z_bins+1)/2  #number of z bin pairs
                ...
                for j1 in xrange(0, self.n_z_bins):
                    ...
                    for i2 in xrange(0,self.n_z_bins):
                        for j2 in xrange(0,self.n_z_bins):
                            if j2<i2: continue
                            cov_pp=data[2*k1*self.n_theta:(2*k1+1)*self.n_theta, 2*k2*self.n_theta:(2*k2+1)*self.n_theta]
                            #print cov_pp
                            cov_mm=data[(2*k1+1)*self.n_theta:(2*k1+2)*self.n_theta,( 2*k2+1)*self.n_theta:(2*k2+2)*self.n_theta]
                            cov_pm=data[2*k1*self.n_theta:(2*k1+1)*self.n_theta, (2*k2+1)*self.n_theta:(2*k2+2)*self.n_theta]
                            cov_mp=data[(2*k1+1)*self.n_theta:(2*k1+2)*self.n_theta, 2*k2*self.n_theta:(2*k2+1)*self.n_theta]
                            cov[k1*self.n_theta:(k1+1)*self.n_theta,k2*self.n_theta:(k2+1)*self.n_theta]=cov_pp
                            cov[(self.n_z_bin_pairs+k1)*self.n_theta:(self.n_z_bin_pairs+k1+1)*self.n_theta,(self.n_z_bin_pairs+k2)*self.n_theta:(self.n_z_bin_pairs+k2+1)*self.n_theta]=cov_mm
                            cov[k1*self.n_theta:(k1+1)*self.n_theta,(self.n_z_bin_pairs+k2)*self.n_theta:(self.n_z_bin_pairs+k2+1)*self.n_theta]=cov_pm
                            cov[(self.n_z_bin_pairs+k1)*self.n_theta:(self.n_z_bin_pairs+k1+1)*self.n_theta,k2*self.n_theta:(k2+1)*self.n_theta]=cov_mp
                            k2+=1
                    k1+=1        
            return cov        
        return data
    ...
    def get_data_vector(self):
            ...
            for j in xrange(i, self.n_z_bins+1):
                ...
                if self.plus_only:
                    if self.theta_mins is not None or self.theta_maxs is not None:
                        ...
                        cut_inds_p = (np.where((theta<theta_min_p) | (theta>theta_max_p)))[0]
                        keep_inds_p= (np.where((theta>=theta_min_p) & (theta<=theta_max_p)))[0]
                        thetas[(j,i)]=[theta[keep_inds_p],[]]
                        thetas_full_p+=list(theta[keep_inds_p])
                        data_vector_p+=list(self.xipm_data['xip_'+bin_comb])
                        cut_inds_p+=k*ntheta
                    ...
                    if (self.theta_mins is not None) or (self.theta_maxs is not None): 
                        theta_min_p,theta_min_m = self.theta_mins[k],self.theta_mins[k+self.n_z_bin_pairs]
                        theta_max_p,theta_max_m = self.theta_maxs[k],self.theta_maxs[k+self.n_z_bin_pairs]
                        ...
                        thetas[(j,i)]=[theta[keep_inds_p],theta[keep_inds_m]]
                        thetas_full_p+=list(theta[keep_inds_p])
                        thetas_full_m+=list(theta[keep_inds_m])
                        data_vector_p+=list(self.xipm_data['xip_'+bin_comb])                        
                        data_vector_m+=list(self.xipm_data['xim_'+bin_comb])
                        cut_inds_p+=k*ntheta
                        keep_inds_p+=k*ntheta
        ...
        cut_inds_full_p,cut_inds_full_m=np.array(cut_inds_full_p),np.array(cut_inds_full_m)
        data_vector_p,data_vector_m=np.array(data_vector_p),np.array(data_vector_m)
        cut_inds_full,data_vector=np.concatenate((cut_inds_full_p,cut_inds_full_m)),np.concatenate((data_vector_p,data_vector_m))
    ...
    def __call__(self, xi_theory_dict):
        ...
        for i in xrange(1, self.n_z_bins+1):
            for j in xrange(i, self.n_z_bins+1):
                ...
                theta_data_p,theta_data_m = self.thetas_arcm[(j,i)][0],self.thetas_arcm[(j,i)][1]
                xi_plus_binned = np.interp(theta_data_p, theta_theory_arcm, xi_plus_theory) 
                xi_minus_binned = np.interp(theta_data_m, theta_theory_arcm, xi_minus_theory)
        ...
        return -chi2/2.0, xi_vector, self.weight, sim
