...
def setup(options):
    ...
    theory_signal_dir = options.get_string(sec, 'theory_signal_dir', default='')
    if theory_signal_dir=='': theory_signal_dir=None
    ...
    if theta_mins=='': theta_mins=None
    else: theta_mins=eval(theta_mins)
    
    print 'Minimum theta values = ',theta_mins
    theta_maxs = options.get_string(sec, 'theta_maxs', default='')
    if theta_maxs=='': theta_maxs=None
    else: theta_maxs=eval(theta_maxs)
    ...
    for i in xrange(1, n_z_bins+1):
        for j in xrange(i, n_z_bins+1):
            ...
            if theory_signal_dir==None:
                ...
                theory_theta=60.*np.degrees(np.loadtxt(os.path.join(theory_signal_dir,'theta.txt')).T)
                theory_xip=np.loadtxt(os.path.join(theory_signal_dir,'xiplus_%d_%d.txt'%(j,i))).T
                theory_xim=np.loadtxt(os.path.join(theory_signal_dir,'ximinus_%d_%d.txt'%(j,i))).T
                xip=np.exp(interp.interp1d(np.log(theory_theta),np.log(theory_xip))(np.log(theta_data)))
                xim=np.exp(interp.interp1d(np.log(theory_theta),np.log(theory_xim))(np.log(theta_data)))
...
def execute(block, config):
    ...
    like, theory_vector, inv_cov, sim = calculator(xi_theory)
