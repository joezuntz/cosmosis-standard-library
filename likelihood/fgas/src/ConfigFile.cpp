...
ConfigFile::ConfigFile( string filename, string delimiter,
                        string comment, string sentry, string include )
  : myDelimiter(delimiter), myComment(comment), mySentry(sentry), myInclude(include)
{
...
std::istream& operator>>( std::istream& is, ConfigFile& cf )
{
  ...
  const string& include = cf.myInclude;
  ...
  while( is || nextline.length() > 0 )
      ...
      if( delimPos < string::npos )
	{
	  ...
	  if (key == include) cf.readFile(line);
	  else cf.myContents[key] = line;  // overwrites if key is repeated
	}
    }
