...
def add_lib_option(libname, opt, default="/usr/local/", install=True):
    ...
    grp.add_option("--%s_islocal" % libname, action="store_true", default=False,
                   help="%s has been installed previsouly using --%s_install or --%s_forceinstall" % (libname, libname, libname))
    grp.add_option("--%s_prefix" % libname, action="store",
                   default="", help="%s include/lib path prefix" % libname)
    grp.add_option("--%s_include" % libname, action="store",
                   default="", help="%s include path" % libname)
    grp.add_option("--%s_lib" % libname, action="store",
                   default="", help="%s lib path" % libname)
    grp.add_option("--%s_link" % libname, action="store",
                   default="", help="%s link line" % libname)
    if install:
        grp.add_option("--%s_installifneeded" % libname, action="store_true", default=False,
                       help="if %s is not found, install it try to install" % libname, dest="%s_install" % libname)
        grp.add_option("--%s_install" % libname, action="store_true", default=False,
                       help="install %s" % libname, dest="%s_forceinstall" % libname)
...
def opt_to_libpaths(ctx, name):
    ...
    include = [v for v in getattr(
        ctx.options, name + "_include", "").split(":") if v]
...
def add_lib(conf, prefix, include, libpath, libname, funcname="", headername="", libs=[], uselib=[], defines=[], frameworkpath=[], framework=[], flagline=""):
    ...
    conf.check_cc(lib=libs, libpath=noemptylist(libpath), rpath=noemptylist(libpath), uselib_store=libname,
                  mandatory=1, uselib=uselib + [libname], defines=defines, frameworkpath=frameworkpath, framework=framework)
    for fnc in funcname:
        conf.check_cc(
...
def shouldIinstall_all(ctx, name):
    if ctx.options.install_all_deps == False:
        return False
    rr = [(vv, getattr(ctx.options, vv, "")) for vv in dir(
        ctx.options) if name in vv and vv != name + "_forceinstall"]
    for vv in rr:
        if vv[1]:
            return False
    return True
...
def upgrade(ctx, name):
    rr = [(vv, getattr(ctx.options, vv, "")) for vv in dir(ctx.options)
          if name in vv and vv not in (name + "_forceinstall", name + "_install")]
    for vv in rr:
        if vv[1]:
            return False
...
def conf_lib(ctx, name, _libs, testfunc=[], testinclude=[], add_inc_path=[], defines=[], frameworkpath=[], framework=[], install=False, msg="", uselib=[], flagline="", opt_name="", add_lib_code="c", forceinstall=False):
    ...
    if install and (upgrade(ctx, name) or forceinstall or getattr(ctx.options, opt_name + "_forceinstall", False) or iall):
        ...
        try:
            ...
            assert forceinstall == False and getattr(
                ctx.options, opt_name + "_forceinstall", False) == False and iall == False
            conf_lib(ctx, name, _libs, testfunc, testinclude, add_inc_path, defines,
                     frameworkpath, framework, False, msg, uselib, flagline, opt_name, add_lib_code)
            return
    ...
    try:
        ...
        if not getattr(ctx.env, "has_" + name, False):
            ...
            Logs.pprint("PINK", "check that %s_prefix or %s_lib and %s_include command line options point toward your %s install" % (
                opt_name, opt_name, opt_name, opt_name))
            Logs.pprint("PINK", "or check that %s is compiled in %d bit" %
                        (name, {True: 64}.get(ctx.options.m64, 32)))
            if msg:
                Logs.pprint("PINK", msg)
            if install:
                Logs.pprint(
                    "PINK", "or install automatically using cmdline option --%s_install" % (opt_name))
            raise e
...
def installsmthg_post(ctx, where, what, extra_config=""):
    ...
    cmdline = "cd build/%s; ./configure --prefix=%s %s  %s %s; make clean;make -j %d ;make install" % (
        where, ctx.env.mprefix, extra_config, CCMACRO, CPPMACRO, ctx.options.jobs)
...
def add_python_option(ctx, name):
    ...
    grp.add_option("--%s_install" % name, action="store_true", default=False,
                   help="install %s" % name, dest="%s_forceinstall" % name)
    grp.add_option("--%s_installifneeded" % name, action="store_true", default=False,
                   help="if %s is not found, install it try to install" % name, dest="%s_install" % name)
...
def configure_python_module(ctx, name, url, packtgz, pack, cmdline=None, extracmd="", forceinstall=False):
    ...
    try:
        ...
        if upgrade(ctx, name) or getattr(ctx.options, name + "_forceinstall", False) or iall:
            ...
            if cmdline == None:
                cmdline = "cd build/%s; PYTHONPATH=%s %s setup.py build_ext -L=%s ;PYTHONPATH=%s %s setup.py install --install-lib=%s --install-scripts=%s" % (
                    pack, ctx.env.PYTHONDIR, ctx.env.PYTHON[0], ctx.env.LIBPATH_PYEMBED[0], ctx.env.PYTHONDIR, ctx.env.PYTHON[0], ctx.env.PYTHONDIR, ctx.env.BINDIR)
            ...
            if (not osp.exists(osp.join(ctx.env.PYTHONDIR, name))) and (not osp.exists(osp.join(ctx.env.PYTHONDIR, name + ".py"))):
                eggdir = [v for v in os.listdir(ctx.env.PYTHONDIR) if name in v and osp.isdir(
                    osp.join(ctx.env.PYTHONDIR, v))][0]
                if eggdir != name and eggdir != name + ".py":
                    mdir = [v for v in os.listdir(
                        osp.join(ctx.env.PYTHONDIR, eggdir)) if name in v][0]
                    import os
                    os.symlink(osp.join(ctx.env.PYTHONDIR, eggdir,
                                        mdir), osp.join(ctx.env.PYTHONDIR, name))
            check_python_module(ctx, name, extracmd)
        else:
            raise e
