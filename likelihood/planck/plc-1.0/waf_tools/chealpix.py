...
def configure(ctx):
    ...
    ctx.env.th = False
    atl.conf_lib(ctx, "chealpix", ["chealpix", "m", "cfitsio"], "pix2vec_ring", "chealpix.h",
                 msg="or check that the path also point toward your cfitsio install", opt_name="healpix", uselib=["cpic", "cfistio"], install=install_healpix)
    atl.conf_lib(ctx, "healpix_f90", ["healpix", "cfitsio"], "HEALPIX_TYPES", msg="or check that the path also point toward your cfitsio install",
                 opt_name="healpix", add_lib_code="f90", uselib=["fpic", "cfistio"], install=install_healpix)
...
def install_healpix(ctx):
    ...
    atl.installsmthg_pre(
        ctx, "http://sourceforge.net/projects/healpix/files/Healpix_2.20a/Healpix_2.20a_2011Feb09.tar.gz/download", "Healpix_2.20a_2011Feb09.tar.gz")
    ...
    dii = {"CC": ctx.env.CC[0], "CFLAGS": " ".join(ctx.env.CCFLAGS + fpic_c + [" -DDONT_USE_PSHT"]), "LIBDIR": ctx.env.LIBDIR, "INCDIR": ctx.env.PREFIX +
           "/include", "FC": ctx.env.FC + " " + " ".join(ctx.env.FCFLAGS + fpic_f90 + [" -DDONT_USE_PSHT"]), "FFLAGS": " "}
