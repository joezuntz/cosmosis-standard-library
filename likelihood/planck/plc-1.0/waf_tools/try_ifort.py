...
def options(ctx):
    ...
    grp.add_option("--gfortran", action="store_true", default=False,
                   help="Do not test for ifort and only use gfortran")
    grp.add_option("--ifort", action="store_true", default=False,
                   help="Do not test for gfortran and only use ifort")
    grp.add_option("--fortran_flagline", action="store", default="",
                   help="flagline to link fortran object to c using ld")
    ...
    if not Options.options.gfortran:
        try:
            ...
            if Options.options.ifort:
                raise
            Logs.pprint(
                "PINK", "ifort not found, defaulting to gfortran (cause: '%s')" % e)
    gfortran_conf(ctx)
...
def configure(ctx):
    ...
    ctx.env.append_value("FCFLAGS_fpic", [
                         flg for flg in ctx.env.FCFLAGS_fcshlib if "-fpic" in flg.lower()])


def show_linkline(ctx):
    ctx.start_msg("fortran link line")
    ctx.end_msg(" ".join(["-L%s" % vv for vv in ctx.env.LIBPATH_fc_runtime]) +
                " " + " ".join(["-l%s" % vv for vv in ctx.env.LIB_fc_runtime]))
...
def ifort_conf(ctx):
    ...
    if not ctx.options.fortran_flagline:
        ...
        try:
            ...
            llgo, llge = ctx.cmd_and_log("%s %s -dryrun -dynamiclib -shared-intel -no-cxxlib dummy.f90" % (
                ctx.env.FC, " ".join(ctx.env.FCFLAGS + ctx.env.FCFLAGS_fc_omp)), output=waflib.Context.BOTH)
            # print "RET",llgo,llge
            L = set([ll.strip() for ll in re.findall("-L(.+)\s*\\\\", llge.split("ld ")
                                                     [1]) if ("ifort" in ll.lower()) or ("intel" in ll.lower())])
            l = set([ll.strip() for ll in re.findall(
                "-l(.+)\s*\\\\", llge.split("ld ")[1])])
            rL = set()
            rl = set()
            for Li in L:
...
def ifort_conf_(ctx):
    ...
    if not ctx.options.fortran_flagline:
        ...
        for pth in res.split(":"):
            ctx.env.append_value("LIBPATH_fc_runtime", pth)
            ctx.env.append_value("RPATH_fc_runtime", pth)
        ctx.env.append_value("LIB_fc_runtime", [
                             "ifcore", "intlc", "ifport", "imf", "irc", "svml", "iomp5", "pthread"])
        ctx.end_msg(True)
    show_linkline(ctx)
...
def gfortran_conf(ctx):
    ...
    v90 = ctx.cmd_and_log(ctx.env.FC + " --version",
                          quiet=Context.STDOUT).split("\n")[0].strip()
    ...
    if len(version90) < 1:
        ...
        if vmid < 3:
            ...
            raise Errors.WafError(
                "gfortran version need to be above 4.3 got %s" % version90)
        ctx.end_msg(v90)
    ...
    lgfpath = ctx.cmd_and_log(
        ctx.env.FC + " %s -print-file-name=libgfortran.dylib" % mopt, quiet=Context.STDOUT)
    lpath = [osp.dirname(osp.realpath(lgfpath))]
    lgfpath = ctx.cmd_and_log(
        ctx.env.FC + " %s -print-file-name=libgomp.dylib" % mopt, quiet=Context.STDOUT)
    lpath += [osp.dirname(osp.realpath(lgfpath))]
    lpath = set(lpath)
