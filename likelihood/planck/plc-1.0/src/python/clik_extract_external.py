...
def main(argv):
    ...
    if "external_dir" in inhf["clik/lkl_%d" % pars.int(default=0).lkl_id].attrs and inhf["clik/lkl_%d" % pars.int(default=0).lkl_id].attrs["external_dir"] == ".":
        import shutil
        shutil.copytree(pars.input_object + "/clik/lkl_%d/_external" %
                        pars.int(default=0).lkl_id, pars.install_path)
    else:
    ...
    if "res_object" in pars:
        ...
        try:
            ...
            pass
        try:
            rmtree(pars.input_object + "/clik/lkl_%d/_external" %
                   pars.int(default=0).lkl_id)
        except Exception as e:
            pass

        outhf["clik/lkl_%d" %
              pars.int(default=0).lkl_id].attrs["external_dir"] = pars.install_path
        outhf.close()
