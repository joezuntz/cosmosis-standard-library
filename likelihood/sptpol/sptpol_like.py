...
class SPTPolTheoryModel(object):
    ...
    def load_windows(self, windows_dir):
        ...
        if self.use_te and self.use_ee:
            ...
            raise ValueError("Must use at least one of te, ee in SPTPol likelihood")
        ...
        if self.use_ee:
            for i in xrange(NBIN):
                filename = os.path.join(windows_dir, "window_{}".format(ee_start+i))
                _, W_i = np.loadtxt(filename).T
                W[i,:,ee_band] = W_i

        if self.use_te:
            for i in xrange(NBIN):
                filename = os.path.join(windows_dir, "window_{}".format(te_start+i))
                _, W_i = np.loadtxt(filename).T
                W[i,:,te_band] = W_i
    ...
    def load_beam_errors(self, beamerr_file):
        ...
        beam_errors = {}
        if self.use_ee:
            beam_errors['ee'] = np.split(errors[:BEAM_TERMS*NBIN], BEAM_TERMS)
        if self.use_te:
            beam_errors['te'] = np.split(errors[BEAM_TERMS*NBIN:], BEAM_TERMS)
        return beam_errors
    ...
    def bandpower_theory_model(self, spectrum, cmb_theory_cl, nuisance_parameters):
        ...
        D2 = self.supersample_lensing(spectrum, cmb_theory_cl, nuisance_parameters)
    ...
    def __call__(self, cmb_theory_cl, nuisance_parameters):
        ...
        if self.use_te:
            B_te, components_te = self.bandpower_theory_model("te", cmb_theory_cl, nuisance_parameters)
            B.append(B_te)

        if self.use_ee:
            B_ee, components_ee = self.bandpower_theory_model("ee", cmb_theory_cl, nuisance_parameters)
            B.append(B_ee)
