...
def configure(ctx):
    atl.conf_lib(ctx, "cfitsio", ["cfitsio"], "fits_init_cfitsio", "fitsio.h",
                 msg="", opt_name="cfitsio", uselib=["cshlib"], install=install_cfitsio)


def install_cfitsio(ctx):
    atl.installsmthg_pre(
        ctx, "ftp://heasarc.gsfc.nasa.gov/software/fitsio/c/cfitsio3280.tar.gz", "cfitsio3280.tar.gz")
    ...
    cmdline = "cd build/%s; ./configure --prefix=%s %s  %s %s; make clean;make -j %d ;make -j %d shared;make install" % (
        "cfitsio", ctx.env.mprefix, "", CCMACRO, CPPMACRO, ctx.options.jobs, ctx.options.jobs)
