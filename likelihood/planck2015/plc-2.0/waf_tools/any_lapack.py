...
lapack_funcs = " ".join(
    file("waf_tools/lapack_funcs.txt").read().strip().split())
...
def options(ctx):
    ...
    grp.add_option("--lapack_mkl", action="store", default="",
                   help="if lapack is mkl, location of the mkl install")
    grp.add_option("--lapack_mkl_version", action="store", default="10.3",
                   help="only needed if version of the mkl library is lower than 10.3 (could be 10.2, 10.1 or 10.0)")
    grp.add_option("--lapack_apple", action="store_true", default=sys.platform.lower()
                   == "darwin", help="use apple version of blas/lapack")
...
def configure(ctx):
    ...
    if ctx.options.lapack_apple:
        ...
        if sys.platform.lower() != "darwin":
            ctx.end_msg("not on darwin ! Got '%s'" %
                        sys.platform, color="YELLOW")
        ...
        if ctx.options.lapack_mkl_version.strip()[:4] not in ("10.0", "10.1", "10.2", "10.3"):
            ctx.end_msg(ctx.options.lapack_mkl_version.strip(
            ) + " not understood, 10.3 library recipe should be ok", color="YELLOW")
            raise Errors.WafError(
                "Cannot understand mkl version '%s'" % ctx.options.lapack_mkl_version.strip())
        ...
        if sys.platform.lower() != 'darwin':
            ...
            cmdlist = {}
            cmdlist["func_list"] = " ".join(
                ["-u %s_" % v for v in lapack_funcs.split()])
            cmdlist["ars"] = " ".join([osp.join(mkl_options[tag][0] % (ctx.options.lapack_mkl), "lib%s.a" % v.strip(
            )) for v in mkl_options[tag][1].split("-l") if v.strip() and v.strip()[:3] == "mkl"])
            cmdlist["Lomp"] = " ".join("-L%s" % v.strip()
                                       for v in ctx.env.LIBPATH_fc_runtime if v.strip())
            cmdlist["omp"] = " ".join(
                [v.strip() for v in mkl_options[tag][1].split() if v.strip() and "mkl" not in v])
            cmdlist["res"] = osp.join(
                ctx.env.LIBDIR, ctx.env.cshlib_PATTERN % "clik_mkl")
            ...
            ctx.options.lapack_lib = ctx.env.LIBDIR + \
                ":".join([""] + ctx.env.LIBPATH_fc_runtime)
            ctx.options.lapack_include = ctx.options.lapack_mkl + "/include"

        else:
            ctx.options.lapack_link = mkl_options[tag][1]
            ctx.options.lapack_lib = mkl_options[tag][0] % (
                ctx.options.lapack_mkl) + ":".join([""] + ctx.env.LIBPATH_fc_runtime)
    ...
    atl.conf_lib(ctx, "lapack", lapack_libs, lapack_funcs.split(),
                 lapack_includes, defines=lapack_extradefs, install=installlapack)
...
def installlapack(ctx):
    ...
    dii = {"FCC": ctx.env.FC, "FCFLAGS": " ".join(ctx.env.FCFLAGS + ctx.env.FCFLAGS_fcshlib), "FLINKFLAGS": " ".join(
        ctx.env.FCFLAGS + ctx.env.LINKFLAGS_fcshlib), "SO": ctx.env.shsuffix, "MFLAG": " ".join(ctx.env.FCFLAGS)}
    ...
    import shutil
    shutil.copyfile("build/%s/liblapack_clik.%s" % (version, ctx.env.shsuffix),
                    osp.join(ctx.env.LIBDIR, "liblapack_clik.%s" % ctx.env.shsuffix))
    shutil.copyfile("build/%s/libblas_clik.%s" % (version, ctx.env.shsuffix),
