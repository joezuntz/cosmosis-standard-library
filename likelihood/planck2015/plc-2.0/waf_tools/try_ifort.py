...
def ifort_conf(ctx):
    ...
    v90 = ctx.cmd_and_log(
        ctx.env.FC[0] + " --version", quiet=Context.STDOUT).split("\n")[0].strip()
    v90 = v90.split("\n")[0].strip().split(" ")[2]
    ctx.end_msg(v90)
    ...
    if not ctx.options.fortran_flagline:
        ...
        try:
            ...
            llgo, llge = ctx.cmd_and_log("%s %s -dryrun -dynamiclib -shared-intel -no-cxxlib dummy.f90" % (
                ctx.env.FC[0], " ".join(ctx.env.FCFLAGS + ctx.env.FCFLAGS_fc_omp)), output=waflib.Context.BOTH)
            # print "RET",llgo,llge
...
def gfortran_conf(ctx):
    ...
    lgfpath = ctx.cmd_and_log(
        ctx.env.FC[0] + " %s -print-file-name=libgfortran.dylib" % (" ".join(mopt)), quiet=Context.STDOUT)
    lpath = [osp.dirname(osp.realpath(lgfpath))]
    lgfpath = ctx.cmd_and_log(
        ctx.env.FC[0] + " %s -print-file-name=libgomp.dylib" % (" ".join(mopt)), quiet=Context.STDOUT)
    lpath += [osp.dirname(osp.realpath(lgfpath))]
    lpath = set(lpath)
