...
def options(ctx):
    ...
    grp.add_option("--gcc", action="store_true", default=False,
                   help="Do not test for icc and only use gcc")
    grp.add_option("--icc", action="store_true", default=False,
                   help="Do not test for gcc and only use icc")
    grp.add_option("--clang", action="store_true", default=False,
                   help="Do not test for gcc and only use clang")
    ctx.add_option_group(grp)


def show_linkline(ctx):
    ctx.start_msg("icc link line")
    ctx.end_msg(" ".join(["-L%s" % vv for vv in ctx.env.LIBPATH_cc_runtime]) +
                " " + " ".join(["-l%s" % vv for vv in ctx.env.LIB_cc_runtime]))
...
def do_icc(ctx):
    ...
    try:
        ...
        llgo, llge = ctx.cmd_and_log("%s %s -dryrun -dynamiclib -shared-intel -no-cxxlib dummy.f90" % (
            ctx.env.CC[0], " ".join(ctx.env.CCFLAGS + ctx.env.CCFLAGS_cc_omp)), output=waflib.Context.BOTH)
        # print "RET",llgo,llge
        L = set([ll.strip() for ll in re.findall("-L(.+)\s*\\\\", llge.split("ld ")
                                                 [1]) if ("ifort" in ll.lower()) or ("intel" in ll.lower())])
        l = set([ll.strip()
                 for ll in re.findall("-l(.+)\s*\\\\", llge.split("ld ")[1])])
        rL = set()
        rl = set()
        for Li in L:
            oli = os.listdir(Li)
...
def do_clang(ctx):
    ...
    v90 = ctx.cmd_and_log(
        ctx.env.CC[0] + " --version", quiet=Context.STDOUT).split("\n")[0].strip()
...
def do_gcc(ctx):
    ...
    if version[0] < 4 or (version[0] == 4 and version[1] < 2):
        ...
        raise Errors.WafError(
            "gcc version need to be above 4.2 got %s" % ".".join(ctx.env.CC_VERSION))
...
def configure(ctx):
    ...
    ctx.env.append_value(
        "CFLAGS_cpic", [flg for flg in ctx.env.CFLAGS_cshlib if "-fpic" in flg.lower()])
