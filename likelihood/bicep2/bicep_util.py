...
def calc_expvals(inpmod_l, inpmod_Cs_l, bpwf_l, bpwf_Cs_l):
    ...
    # TT
    x = bpwf_Cs_l[:, :, 0] * \
        np.transpose(np.tile(inpmod_Cs_l[indx, 0], (nbin, 1)))
    expv[:, 0] = np.sum(x, 0)

    # TE
    x = bpwf_Cs_l[:, :, 1] * \
        np.transpose(np.tile(inpmod_Cs_l[indx, 1], (nbin, 1)))
    expv[:, 1] = np.sum(x, 0)

    # EE: x1 = EE->EE, x2 = BB->EE
    x1 = bpwf_Cs_l[:, :, 2] * \
        np.transpose(np.tile(inpmod_Cs_l[indx, 2], (nbin, 1)))
    x2 = bpwf_Cs_l[:, :, 5] * \
        np.transpose(np.tile(inpmod_Cs_l[indx, 3], (nbin, 1)))
...
def calc_vecp(l, C_l_hat, C_fl, C_l):
    ...
    [d, u] = LA.eigh(res)
    # noticed that python returns the eigenvalues as a vector, not a matrix
    d = np.diag(d)
    ...
    gd = np.sign(np.diag(d) - 1) * np.sqrt(2 *
                                           (np.diag(d) - np.log(np.diag(d)) - 1))
    gd = np.diag(gd)
...
def saveLikelihoodToText(rlist, logLike, field, exp='bicep1'):
    ...
    if exp == 'bicep1':
        ...
        f.write(
            '# This file is generated from a standalone python module: b1_r_wrapper.py \n')
        ...
        f.write(
            '# Based on data from: DETECTION OF B-mode POLARIZATION AT DEGREE SCALES USING BICEP2 \n')
        ...
        f.write(
            '# Calculated via the "Hamimeche-Lewis likelihood" method described in Sectiox [specify seciotn and paper title/author here.] \n')
        f.write(
            '# This file is generated from a standalone python module: bicep_r_wrapper.py \n')
        f.write('# \n')
        f.write('# Columns:  r, logLiklelihood(r) \n')
...
def init(experiment, field):
    ...
    for ell in range(0, 9):
        for ellp in range(0, 9):
            M_inv[ell, ellp, :, :] = M_invp[ell *
                                            dim:(ell + 1) * dim, ellp * dim:(ellp + 1) * dim]
