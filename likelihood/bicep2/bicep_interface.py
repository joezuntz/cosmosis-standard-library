...
def setup(options):
	...
	with working_directory(dirname):
		...
		C_l, C_l_hat, N_l, C_fl, M_inv, bpwf_l, bpwf_Cs_l = bu.init(experiment,field)
		...
		assert int(ell[0])==0, "BICEP data format error - something has changed - see code"
...
def execute(block, config):
	...
	this_mod[theory_ell_min:data_ell_max+1,2] = block[cmb_cl, "EE"][:data_ell_max-1]
	this_mod[theory_ell_min:data_ell_max+1,3] = block[cmb_cl, "BB"][:data_ell_max-1]
