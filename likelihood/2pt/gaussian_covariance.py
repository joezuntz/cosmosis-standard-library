...
def compute_gaussian_covariance(sky_area, get_theory_cl, block, AB, CD):
    ...
    for X in (A,B,C,D):
        assert X in ["galaxy_shear_emode_fourier", "galaxy_position_fourier"], "Have not yet coded up gaussian covariance for spectra other than pos-pos, shear-shear, shear-pos"
    ...
    for x,y,ell in find_equal_ell(ell_AB,ell_CD):
        ...
        if delta_AB is None:
            warnings.warn("There are no delta-ell values for some combinations of ({},{}).".format(A,B))
            delta_AB = delta_ell_AB.values()[0]
        if delta_CD is None:
            warnings.warn("There are no delta-ell values for some combinations ({},{}).".format(C,D))
