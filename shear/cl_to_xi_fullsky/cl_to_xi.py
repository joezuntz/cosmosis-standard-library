...
def get_N_ell(ell):
    """N_ell (eq. 2.16)"""
    N_ell = np.atleast_1d(np.sqrt(2./(ell-1)/ell/(ell+1)/(ell+2)))
    N_ell[ell<2] = 0.
...
def cl_to_xi_plus_and_minus_precomp(cl_input, thetas, G_plus_minus_pre):
    ...
    for i,theta in enumerate(thetas):
        ...
        C_plus[i] = np.sum(((2*ells+1)/2./pi) * N_ell**2 * Cls * G_plus_pre[i]) / 2
        C_cross[i] = np.sum(((2*ells+1)/2./pi) * N_ell**2 * Cls * G_minus_pre[i]) / 2
...
def save_xi_22(block, section, bin_i, bin_j, cl_input, thetas, G_plus_minus_pre):
    xi_plus, xi_minus = cl_to_xi_plus_and_minus_precomp(cl_input, thetas, G_plus_minus_pre)
