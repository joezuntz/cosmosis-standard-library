...
def P2l_rec(ells,cost):
    ...
    for ell in ells[4:]:
        ...
        P2l[ell] = ((2*ell-1)*cost*P2l[ell-1]-(ell+2-1)*P2l[ell-2])/(ell-2)
    return P2l
...
def P2l_rec_norm(ells,cost):
    ...
    for ell in ells[4:]:
        ...
        c = np.sqrt(((ell-1.)**2-4)/(4*(ell-1.)**2-1)) * P2l_norm[ell-2]
        #print a,b,c
        P2l_norm[ell] = a * ( b - c )
...
def precomp_GpGm(ells, thetas):
    ...
    G_plus  = -((ELLS-4)/SINTS**2 + 0.5*ELLS*(ELLS-1))*P_m_l + (ELLS+2)*COSTS*P_m_lminus1/SINTS**2
    G_minus = 2 * ((ELLS-1)*COSTS*P_m_l - (ELLS+2)*P_m_lminus1)/SINTS**2
    G_plus[:,0]=0.
    G_minus[:,0]=0.
...
def G_plus_minus_l2(ells,theta,s_switch=100.):
    ...
    P_m_l[~do_full] = (ells[~do_full]-1)*ells[~do_full]*(ells[~do_full]+1)*(ells[~do_full]+2) * sp.jn(2, ells[~do_full]*theta) / ells[~do_full]**2
    print 'theta = %f, switching to bessel function for %d<ell<%d'%(np.degrees(theta)*60.,ells[~do_full][0],ells[~do_full][-1])
    P_m_lminus1[1:]=P_m_l[:-1]
    G_plus  = -((ells-4)/sint**2 + 0.5*ells*(ells-1))*P_m_l + (ells+2)*cost*P_m_lminus1/sint**2
    G_minus = 2 * ((ells-1)*cost*P_m_l - (ells+2)*P_m_lminus1)/sint**2
    G_plus[0]=0.
    G_minus[0]=0.
    return G_plus, G_minus
